class ValidatorView {
  ...
  addListeners() {
    var self = this;
    this.$title.on('keyup', function(e) {
      self.model.setTitle($(e.currentTarget).val()) 
    })
    
    this.$body.on('keyup', function(e) {
      self.model.setBody($(e.currentTarget).val())
    })
...
class ValidatorModel {
  ...
  valid() {
    ...
    if (this.title.length < this.titleMinLength) {
      this.errors.push('タイトルは' + this.titleMinLength + '文字以上でおなしゃす')
    }
    
    if (this.title.length > this.titleMaxLength) {
      this.errors.push('タイトルは' + this.titleMaxLength + '文字を超えています')
    }
